...
## OAuth Multi-Tenant / On-Behalf → Separate Orgs or API Keys

**BoldSign:** OAuth 2.0 lets your app act on behalf of other BoldSign accounts, and
`onBehalfOf` / brand sends let one account send as a teammate or brand
(`BehalfDocument*` events).

**Anvil:** No OAuth-on-behalf equivalent. Each Anvil organization has its own API
key; sender identity is the org (customizable via `replyToName`/`replyToEmail`).

**Workaround:**
1. **Single org with template separation** — one Anvil account; track which
   templates/brands belong to which tenant in your database.
...
brand IDs from your send calls.
...
| OAuth multi-tenant / on-behalf | Separate orgs / keys | High |
